...
import { Plus, Trash2, Loader2, Scale, ChevronDown, ChevronUp } from "lucide-react";
            ...
            <Card key={m.id} className="group">
              <CardContent className="p-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="font-medium text-sm">{formatRelativeDate(m.date)}</p>
                    <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2">
                      {m.weightKg && <MetricPill label="Weight" value={`${m.weightKg} kg`} />}
                      {m.bodyFatPct && <MetricPill label="BF%" value={`${m.bodyFatPct}%`} />}
                      {m.waistCm && <MetricPill label="Waist" value={`${m.waistCm} cm`} />}
                      {m.hipCm && <MetricPill label="Hip" value={`${m.hipCm} cm`} />}
                      {m.chestCm && <MetricPill label="Chest" value={`${m.chestCm} cm`} />}
                      {m.armCm && <MetricPill label="Arm" value={`${m.armCm} cm`} />}
                    </div>
                    {m.notes && (
                      <p className="text-xs text-[var(--muted-foreground)] mt-2">{m.notes}</p>
                    )}
                  </div>
                  <button
