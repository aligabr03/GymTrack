...
function makeSet(exerciseId: string, exerciseName: string, setNumber: number): SetRow {
  return {
    id: makeId(),
    exerciseId,
    exerciseName,
...
export function WorkoutLogger({
        ...
        return prev.map((g) =>
          g.exerciseId === exercise.id
            ? {
                ...
                sets: [
                  ...g.sets,
                  makeSet(exercise.id, exercise.name, g.sets.length + 1),
                ],
              }
            : g
        );
    ...
    setGroups((prev) =>
      prev.map((g) =>
        g.exerciseId === exerciseId
          ? {
              ...g,
              sets: [...g.sets, makeSet(exerciseId, g.exerciseName, g.sets.length + 1)],
            }
          : g
      )
    );
  ...
  function updateSet(exerciseId: string, setId: string, field: keyof SetRow, value: string | number | null) {
    setGroups((prev) =>
      prev.map((g) =>
        g.exerciseId === exerciseId
          ? {
  ...
  function toggleCollapse(exerciseId: string) {
    setGroups((prev) =>
      prev.map((g) =>
        g.exerciseId === exerciseId ? { ...g, collapsed: !g.collapsed } : g
      )
    );
  }
  ...
  function handleSave() {
    ...
    if (allSets.length === 0) {
      toast({ title: "Add at least one set to save", variant: "destructive" });
      return;
    }
    ...
    startTransition(async () => {
      ...
      toast({ title: existing ? "Workout updated" : "Workout saved! 💪", variant: "success" });
      router.push("/workouts");
    });
  }

  const totalSets = groups.reduce((s, g) => s + g.sets.length, 0);
  const totalVolume = groups.reduce((total, g) =>
    total + g.sets.reduce((s, set) => {
    ...
    <div className="space-y-6">
      ...
      {groups.length > 0 && (
        <div className="flex items-center gap-6 px-1 text-sm text-[var(--muted-foreground)]">
          <span>
            <strong className="text-[var(--foreground)]">{groups.length}</strong> exercises
          </span>
          <span>
            <strong className="text-[var(--foreground)]">{totalSets}</strong> sets
          </span>
          <span>
            <strong className="text-[var(--foreground)]">{Math.round(totalVolume).toLocaleString()} kg</strong> volume
          </span>
        </div>
      )}
      ...
      {groups.map((group) => (
        <ExerciseGroupCard
          ...
          onUpdateSet={(setId, field, value) => updateSet(group.exerciseId, setId, field, value)}
                ...
                filteredExercises.map((ex) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-[var(--muted-foreground)]">{ex.category}</p>
                    </div>
      ...
      <div className="flex justify-end gap-3 pt-4 border-t border-[var(--border)]">
        ...
        <Button onClick={handleSave} disabled={isPending || groups.length === 0}>
          {isPending ? (
            <>
...
function ExerciseGroupCard({
  ...
  onUpdateSet: (setId: string, field: keyof SetRow, value: string | number | null) => void;
    ...
    <div className="space-y-1">
      <div className="grid grid-cols-[2rem_1fr_1fr_1fr_1fr_2rem] gap-2 items-center">
        ...
        <Select
          ...
          onValueChange={(v) => onUpdate("formRating", v ? parseInt(v, 10) : null)}
        >
