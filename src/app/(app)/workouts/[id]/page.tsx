  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <SummaryCard label="Volume" value={`${Math.round(volume).toLocaleString()} kg`} />
        <SummaryCard label="Sets" value={workout.sets.length.toString()} />
        <SummaryCard label="Exercises" value={Object.keys(byExercise).length.toString()} />
        {workout.durationMins && (
          <SummaryCard label="Duration" value={`${workout.durationMins} min`} icon={<Clock className="h-4 w-4" />} />
        )}
      </div>
      ...
      {workout.notes && (
        <Card>
          <CardContent className="p-4">
            <p className="text-sm text-[var(--muted-foreground)]">{workout.notes}</p>
          </CardContent>
        </Card>
      )}
            ...
            <Card key={exercise.id}>
              ...
              <CardContent className="pt-0">
                <div className="space-y-2">
                  ...
                  {sets.map((s) => {
                    const formLabel = FORM_RATINGS.find((r) => r.value === s.formRating);
                    return (
                      <div
                        key={s.id}
                        className="grid grid-cols-[2rem_1fr_1fr_1fr_1fr] gap-2 items-center px-1 py-1 rounded hover:bg-[var(--secondary)] transition-colors text-sm"
                      >
                        <span className="text-xs text-[var(--muted-foreground)] font-mono">
                          {s.setNumber}
                        </span>
                        <span>{s.weightKg ? `${s.weightKg} kg` : "—"}</span>
                        <span>{s.reps ?? "—"}</span>
                        <span>
                          {formLabel ? (
                            <Badge
